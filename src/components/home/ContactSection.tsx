...
const ContactSection = () => {
  ...
  const EMAILJS_SERVICE_ID = "service_zhllzg8";
  ...
  const onSubmit = async (data: ContactFormValues) => {
    ...
    try {
      ...
      console.error("Error submitting form:", err?.message || err?.text || error);
      setStatus({
        type: "error",
      ...
      toast({
        title: "Error",
        description: err?.text || err?.message || "Failed to send message. Please try again later.",
        variant: "destructive",
      });
    } finally {
